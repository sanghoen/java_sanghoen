...
public class LoanBrowsing implements Serializable{
	...
	private Date estimateDate;//반납예정일
	
	public LoanBrowsing(Book book, Date date, int loanPeriod) {
		this.book = book;
		this.loanDate = date;
		//대출 기한을 이용하여 반납 예정일을 추가
		calculateReturnData(loanPeriod);
	}
	
	private void calculateReturnData(int loanPeriod) {
		if(loanDate == null) {
			return;
		}
		 ...
		 * */
		estimateDate = new Date(loanDateMs + periodMs);
	...
	public String getEstimateDateStr() {
		SimpleDateFormat format = new SimpleDateFormat("yyyy년 MM월 dd일");
		return format.format(estimateDate);
	}
