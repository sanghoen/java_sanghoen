...
=> 데이터의 일관성을 유지하기 위해
트리거 예시
- 쇼핑몰에서 회원이 주문을 완료하면 주문 리스트 테이블에 데이터가 추가 => 옵션 수량이 변경
트리거 특징
...
-- 주문 테이블에 데이터가 추가 => 주문 리스트에 데이터가 추가 => 각 주문 리스트에 맞는 옵션에 수량이 변경
...
BEGIN
	UPDATE `OPTION` 
	SET
		OL_AMOUNT = OL_AMOUNT - NEW.OL_AMOUNT
	WHERE -- 조건
		OP_NUM = NEW.OL_OR_NUM;
END //
DELIMITER ;
