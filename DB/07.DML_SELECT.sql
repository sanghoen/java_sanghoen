...
=> 조건식을 만족하는 행(튜플)들만 검색해서 해당햇의 속성1,...,속성n의 정보를 조회
where절
속성명1 = 값1 or 속성명2 = 값2 and 속성명 = 값3
where절의 사용되는 연산자
= : 같다. null과가 비교가 안됨
!= : 같지 않다. null과가 비교가 안됨
<> : 같지 않다. null과가 비교가 안됨
...
=> 속성명 >= A and 속성명 <= B  --같은 의미 
속성명 in(값1, 값2, ..., 값n) : 속성의 값이 값1이거나 값2이거나 ... 값n인 조건을 만족할 때 사용
=> 속성명 = 값1 or 속성명 = 값2 or ... or 속성명 = 값n  --같은 의미
*/
/*
distinct : 중복된 결과를 제거하고 조회
select distinct 속성명 form 테이블명;
*/
/*
- ANY
where 속성명 비교연산자(=, >=, <=, !=, <, >) any(서브쿼리);
=> 속성이 서브쿼리 결과와 비교연산을 했을 때 서브쿼리 결과중 하나이상 만족하면 조회
where 속성명 = any(서브쿼리)
=> where 속성명 in(서브쿼리) 
...
=> 속성명1의 정렬방법을 기준으로 정렬하고, 속성명1의 값이 같으면 속성명2를 정렬방법을 기준으로 정렬...
- 정렬방법
...
-- 이순신교수님과 한석봉교수님이 강의하는 수강정보를 조회하는 쿼리
...
-- 학점이 2학점 또는 3학점 이상인 수업을 과목의 수강 정보를 조회
...
-- 학생 정보를 이름순으로 정렬하여 조회
select * from student order by name asc;  -- asc 생략가능
...
-- 학생들 정보를 학번순으로 오름차순 정렬 후, 위에서 3번째 4번째 학생 정보를 조회하는 쿼리
select * from student limit 2, 2;
-- 학생들 정보를 학번순으로 오름차순 정렬 후, 2명씩 보여줄 때 3번째 페이지 있는 학생 정보를 조회하는 쿼리
