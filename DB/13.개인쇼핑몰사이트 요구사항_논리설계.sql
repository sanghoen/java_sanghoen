...
나
- 고객은 회원가입을 통해 프로그램을 이용한다.
- 고객은 회원가입시, 이름, 생년월일을 입력한다.
- 제품을 등록한다.
- 제품의 제품번호, 제품명, 가격, 입고일 등을 입력할 수 있다.
- 고객은 제품을 검색할 수 있다.
- 고객은 제품을 구매할 수 있다.
- 판매된 물건들의 판매 갯수와 수익을 기록할 수 있다.
...
- 회원 가입시 아이디는 중복 체크를 하고, 전화번호를 통해 본인인증을 한다.
...
- 회원은 리뷰 등록시, 별점(1~5)과 리뷰 내용, 이미지(최대 하나)를 등록한다.

- 회원은 제품은 검색해서 제품명, 가격, 별점을 조회할 수 있다.
...
두 개체의 관계가 1:n인 경우 - 관계에 속성이 없는 경우
	=> n쪽인 개체에 다른 개체의 기본키를 외래키를 설정.
-- 관계에 속성이 있는 경우
두 개체의 관계가 1:1, 1:n, n:m 상관 없음
	=> 새 릴레이션을 추가 후, 관계에 있는 속성을 새 릴레이션에 추가하고, 각 개체의 기본키들을
	   새 릴리이션의 외래키로 지정
*/
/*
