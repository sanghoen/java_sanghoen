...
-- 주소가 서울시 강남구 KH빌딩, 주소 상세가 6층 361반, 우편번호가 01234인 회원이 가입을 했을 때 쿼리
-- 권한은 ADMIN, USER 중 하나
...
INSERT INTO MEMBER(ME_ID, ME_PW, ME_PHONE, ME_ADDR, ME_ADDR_DETAIL, ME_POST, ME_AUTHORITY, ME_NAME)
	VALUES('admin', 'admin', '011-1111-1111', '서울시 강남구 KH빌딩','3층 101반','01234', 'ADMIN', '임꺽정');     
INSERT INTO MEMBER(ME_ID, ME_PW, ME_PHONE, ME_ADDR, ME_ADDR_DETAIL, ME_POST, ME_AUTHORITY, ME_NAME)
...
values('집', '서울기 강남구 123번지', 'KH아파트 101동 101호', '12345', 'abc123');
...
-- 제품 코드가 abc002, 제품명이 kh키보드, 제품상세가 혁신적인 kh 키보드, 제품가격이 30000원
-- 제품 종류가 '기계식', '일반'인 주품을 등록하는 쿼리
-- 1. 제품 추가
insert into product values('abc002', 'kh키보드', '혁신적인 kh 키보드', 30000);
-- 2. 옵션 추가
...
-- 배송지를 집으로, 경기도 안산시 안산구 123번지, 상세는 없음, 우편번호 12312를 등록 후 구매 
-- 0. 배송지 테이블에 추가
insert into address (ad_name, ad_addr, ad_addr_detail, ad_post, ad_me_id)
...
-- 포인트 내역에 적립 내용을 추가
	...
	values('제품 구매에 의한 적립', 3000, 'abc123');
-- 회원 정보에 포인트를 누적
update member set me_point = me_point + 3000 where me_id ='abc';

-- abc회원이 구매 확정한 abc001제품중에 무선인 제품에 '좋은 마우스입니다.'라고 리뷰 등록했을 때 쿼리
insert into review(re_content, er_thumb, re_op_num, re_me_id)
