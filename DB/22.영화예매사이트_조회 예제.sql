...
SELECT DISTINCT
	...
	MOVIE ON MS_MO_NUM = MO_NUM
WHERE
	NOW() < CONCAT(MS_DATE,' ', MS_START_TIME) -- 2023-08-16 09:00:00
...
-- 사용 금액이 같은 회원이 여러명인 경우, 누구는 순위로 포함되고 누구는 순위에 포함되지 않을 수 있다.
...
FROM
	FILM_PERSON
	LEFT JOIN
	(SELECT * FROM ROLE WHERE RO_ROLE = '배우') AS ROLE2 
    ON RO_FP_NUM = FP_NUM
GROUP BY FP_NUM;
...
FROM
    MOVIE_SCHEDULE
        JOIN
    SCREEN ON MS_SC_NUM = SC_NUM -- 영화관과 연결을 위해 먼저 영상관과 연결
		JOIN
	THEATER ON TH_NUM = SC_TH_NUM -- 영화관명을 조회하기 위해 영화관과 연결
		JOIN
	MOVIE ON MS_MO_NUM = MO_NUM -- 영화 제목을 조회하기 위해 영화관과 연결
WHERE
	NOW() < CONCAT(MS_DATE, ' ', MS_START_TIME)   
